...
public class DAO_usuario {
    ...
    private String sql_insert = "INSERT INTO public.usuario(\n"
            + "            idusuario, nombre, usuario, senha, nivel,fk_idcajero)\n"
            + "    VALUES (?, ?, ?, ?, ?,?);";
    private String sql_update = "UPDATE public.usuario\n"
            + "   SET  nombre=?, usuario=?, senha=?, nivel=?,fk_idcajero=? \n"
            + " WHERE idusuario=?;";
    private String sql_select = "select u.idusuario as idu, u.nombre, u.usuario,c.nombre as cajero "
            + "from usuario u,cajero c "
            + "where u.fk_idcajero=c.idcajero "
            + "order by 1 asc";
    private String sql_cargar = "SELECT idusuario, nombre, usuario, senha, nivel,fk_idcajero \n"
    ...
    public void cargar_usuario(usuario usu, JTable tabla) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                usu.setU6fk_idcajero(rs.getInt("fk_idcajero"));
            }
        } catch (Exception e) {
    ...
    public void insertar_usuario(Connection conn, usuario usu) {
        ...
        try {
            ...
            pst.setInt(6, usu.getU6fk_idcajero());
            pst.execute();
            pst.close();
    ...
    public void update_usuario(Connection conn, usuario usu) {
        ...
        try {
            ...
            pst.setInt(5, usu.getU6fk_idcajero());
            pst.setInt(6, usu.getU1idusuario());
            pst.execute();
            pst.close();
    ...
    public boolean getBoolean_buscar_usuario_existente(Connection conn, usuario usu) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                usu.setGlobal_idcajero(rs.getInt("fk_idcajero"));
                return true;
            }else{
