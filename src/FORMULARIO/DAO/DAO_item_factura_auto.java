...
public class DAO_item_factura_auto {
    ...
    public void insertar_item_factura_auto_de_venta(Connection conn, int idventa) {
        String sql = "INSERT INTO item_factura_auto( fecha_creado, creado_por, descripcion, cantidad, \n"
                ...
                + "       case when iv.tipo='P' then iv.iva\n"
                + "            when iv.tipo='N' then iv.iva\n"
                + "            when iv.tipo='I' then 10\n"
                + "            when iv.tipo='D' then 10\n"
                + "            else 0 end as iva,\n"
                + " case when iv.tipo='P' and iv.iva=0 then (iv.precio_venta*iv.cantidad) \n"
                + "      when iv.tipo='N' and iv.iva=0 then (iv.precio_venta*iv.cantidad) \n"
                + "      else 0 end as total_exenta,\n"
                + " case when iv.tipo='P' and iv.iva=5 then (iv.precio_venta*iv.cantidad) \n"
                + "      when iv.tipo='N' and iv.iva=5 then (iv.precio_venta*iv.cantidad) \n"
                + "      else 0 end as total_gravada_5,\n"
                + " case when iv.tipo='P' and iv.iva=10 then (iv.precio_venta*iv.cantidad) \n"
                ...
                + "case when iv.tipo='P' and iv.iva=0 then 'CUNETA exenta' \n"
                + "      when iv.tipo='N' and iv.iva=0 then 'CUNETA exenta' \n"
                + "      else '---' end as cuenta_exenta,\n"
                + " case when iv.tipo='P' and iv.iva=5 then 'CUNETA 5 iva' \n"
                + "      when iv.tipo='N' and iv.iva=5 then 'CUNETA 5 iva' \n"
                + "      else '---' end as cuenta_5,\n"
                + " case when iv.tipo='P' and iv.iva=10 then 'CUNETA 10 iva' \n"
                + "      when iv.tipo='N' and iv.iva=10 then 'CUNETA 10 iva' \n"
                ...
                + "case when iv.tipo='P' then iv.fk_idproducto\n"
                + "     when iv.tipo='N' then iv.fk_idproducto\n"
                + "     when iv.tipo='I' then 0\n"
                + "     when iv.tipo='D' then 0\n"
                + "     else 0 end as fk_idproducto\n"
                + "  FROM item_venta iv\n"
                + "where  iv.precio_venta!=0 and iv.fk_idventa="+idventa
