...
public class DAO_caja_detalle {
    ...
    private String sql_update_cerrartodo="update caja_detalle set cierre='C' "
                + "where cierre='A' ";
    private String sql_cambiar_monto="update caja_detalle set "
                + "monto_venta_efectivo=?,monto_venta_tarjeta=? "
                + "where tabla_origen=? and id_origen=?;";
    ...
    public void cambiar_monto_caja_detalle(Connection conn, caja_detalle caja){
        String titulo = "cambiar_monto_caja_detalle";
        PreparedStatement pst = null;
        try {
            pst = conn.prepareStatement(sql_cambiar_monto);
            pst.setDouble(1, caja.getC4monto_venta_efectivo());
            pst.setDouble(2, caja.getC18monto_venta_tarjeta());
            pst.setString(3, caja.getC10tabla_origen());
            pst.setInt(4,caja.getC9id_origen());
            pst.execute();
            pst.close();
