...
//    public void update_estado_venta_servidor(Connection conn, String indice, String estado) {
//        String titulo = "update_estado_venta_servidor";
//        try {
//            if (conn.getAutoCommit()) {
//                conn.setAutoCommit(false);
//            }
//            vdao.update_estado_venta_servidor(conn, indice, estado);
//            conn.commit();
//        } catch (SQLException e) {
//            evmen.mensaje_error(e, indice, titulo);
//            try {
//                conn.rollback();
//            } catch (SQLException e1) {
//                evmen.Imprimir_serial_sql_error(e1, indice, titulo);
//            }
//        }
//    }
    ...
    public void cambiar_monto_venta(Connection conn, venta ven, caja_detalle caja) {
        String titulo = "cambiar_monto_venta";
        try {
            if (conn.getAutoCommit()) {
                conn.setAutoCommit(false);
            }
            vdao.cambiar_monto_pago_venta(conn, ven);
            cdao.cambiar_monto_caja_detalle(conn, caja);
            conn.commit();
        } catch (SQLException e) {
            evmen.mensaje_error(e, ven.toString(), titulo);
            try {
