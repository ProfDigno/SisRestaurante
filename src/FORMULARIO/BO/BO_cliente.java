...
public class BO_cliente {
    ...
    public void update_cliente(cliente clie, JTable tbltabla) {
        if (evmen.MensajeGeneral_warning("ESTAS SEGURO DE MODIFICAR ESTE CLIENTE", "MODIFICAR", "ACEPTAR", "CANCELAR")) {
            ...
            try {
                ...
                pidao.update_cliente(conn, clie,true);
                pidao.actualizar_tabla_cliente(conn, tbltabla);
                conn.commit();
            } catch (SQLException e) {
                evmen.mensaje_error(e, clie.toString(), titulo);
                try {
                    conn.rollback();
                } catch (SQLException e1) {
                    evmen.Imprimir_serial_sql_error(e1, clie.toString(), titulo);
                }
            }
        }
    }

    public void update_cliente_enventa(cliente clie) {
        String titulo = "update_cliente_enventa";
        Connection conn = ConnPostgres.getConnPosgres();
        try {
            if (conn.getAutoCommit()) {
                conn.setAutoCommit(false);
            }
            pidao.update_cliente(conn, clie,false);
            conn.commit();
        } catch (SQLException e) {
            evmen.mensaje_error(e, clie.toString(), titulo);
            try {
